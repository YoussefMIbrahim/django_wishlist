...
class TestViewHomePageIsEmptyList(TestCase):
    ...
    def test_load_wishlist_page_shows_empty_list(self):
        ...
        self.assertTemplateUsed(response, 'trave_wishlist/wishlist.html')
        self.assertEquals(0, len(response.context['places']))

    def test_load_visted_page_shows_empty_list(self):
        response = self.client.get(reverse('places_visited'))
        self.assertTemplateUsed(response, 'trave_wishlist/visited.html')
...
class TestWishList(TestCase):
    ...
    def test_view_wishlist(self):
        ...
        data_expected = list(Place.objects.filter(user=self.user).filter(visted=False))
    ...
    def test_view_places_visited(self):
        ...
        data_expected = list(Place.objects.filter(user=self.user).filter(visted=True))
        # Is it the same?
        self.assertCountEqual(data_rendered, data_expected)
...
class TestAddNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place_to_wishlist(self):
        ...
        response = self.client.post(reverse('place_list'), { 'name': 'Tokyo', 'visted': False }, follow=True)
        ...
        tokyo_in_database = Place.objects.get(name="Tokyo", visted=False)
        ...
        response =  self.client.post(reverse('place_list'), { 'name': 'Yosemite', 'visted': False }, follow=True)
        ...
        # the values expected. Will throw an exception if no data, or more than
        # one row, matches. Remember throwing an exception will cause this test to fail
        place_in_database = Place.objects.get(name="Yosemite", visted=False)
        place_in_database = Place.objects.get(name="Tokyo", visted=False)
    ...
    def test_add_new_visited_place_to_wishlist(self):
        ...
        response = self.client.post(reverse('place_list'), { 'name': 'Tokyo', 'visted': True }, follow=True)
        ...
        # the values expected. Will throw an exception if no data, or more than
        # one row, matches. Remember throwing an exception will cause this test to fail
        place_in_database = Place.objects.get(name="Tokyo", visted=True)
...
class TestPlaceDetail(TestCase):
    ...
    def test_place_detail(self):
        ...
        # Check correct template was used
        self.assertTemplateUsed(response, 'trave_wishlist/place_detail.html')
